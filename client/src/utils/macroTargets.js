export function calculateMacroTargets(calorieGoal, goal = "maintain", profile = {}) {
...
export function macroProgress(consumed, target) {
  const safeTarget = Math.max(Number(target) || 0, 1);
  const value = Number(consumed) || 0;
  return Math.max(0, Math.min((value / safeTarget) * 100, 100));
}
