...
function AnimatedMacroCard({ macro, delayMs = 0 }) {
  ...
  const percentage = macro.target > 0 ? Math.min((currentValue / macro.target) * 100, 100) : 0;
  const isOverTarget = currentValue > macro.target;

  return (
    <div
      className="relative p-4 rounded-2xl overflow-hidden transition-all duration-300 hover:scale-[1.02] active:scale-[0.98]"
      style={{
        background: `linear-gradient(135deg, ${macro.color}08 0%, ${macro.color}03 100%)`,
        border: `1.5px solid ${macro.color}20`,
        boxShadow: `0 4px 12px ${macro.color}08`,
      }}
    >
      {/* Background Circle Decoration */}
      <div 
        className="absolute -right-6 -top-6 w-24 h-24 rounded-full opacity-5"
      ...
      >
        <span className="text-lg font-black" style={{ color: macro.color }}>
          {macro.label.charAt(0)}
        </span>
      </div>

      {/* Label */}
      <p className="text-xs font-bold uppercase tracking-wider mb-1 relative z-10" 
      ...
      <div className="flex items-baseline gap-1 mb-3 relative z-10">
        <p className="text-2xl font-black tabular-nums" style={{ color: macro.color }}>
          {currentValue}
        </p>
        <span className="text-xs font-bold opacity-60" style={{ color: macro.color }}>
          / {macro.target}g
        </span>
      </div>
        ...
        <div
          ...
          style={{
            ...
            background: isOverTarget 
              ? `linear-gradient(90deg, ${macro.color} 0%, #ef4444 100%)`
              : `linear-gradient(90deg, ${macro.color}80 0%, ${macro.color} 100%)`,
